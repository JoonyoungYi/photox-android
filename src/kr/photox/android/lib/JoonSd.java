...
public class JoonSd extends SlidingDrawer {
	...
	public JoonSd(Context context, AttributeSet attrs) {
		super(context, attrs);
	}

	public JoonSd(Context context, AttributeSet attrs, int defStyle) {
	...
	private boolean isAnyClickableChildHit(ViewGroup viewGroup, int clickX,
			int clickY) {
		for (int i = 0; i < viewGroup.getChildCount(); i++) {
			...
			if (childView instanceof ViewGroup
					&& isAnyClickableChildHit((ViewGroup) childView, clickX,
							clickY)) {
				return true;
			}
		}
